...
class CustomerService {
	...
	async createCustomer(customerCardSchema: CustomerCardSchemaType): Promise<CustomerCard> {
	...
	async updateCustomer({
												 customerNumber,
												 ...customerSchema
											 }: { customerNumber: string } & CustomerCardSchemaType): Promise<CustomerCard> {
		const response = await fetch(`${API_BASE_URL}customer-card/${customerNumber}`, {
			method: 'PATCH',
			headers: {
				'Content-Type': 'application/json',
			},
	...
	async deleteCustomer(customerCardNumber: string): Promise<void> {
		const response = await fetch(`${API_BASE_URL}customer-card/${customerCardNumber}`, {
			method: 'DELETE',
			headers: {
				'Content-Type': 'application/json',
			},
