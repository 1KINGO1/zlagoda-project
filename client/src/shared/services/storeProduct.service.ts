...
class StoreProductService {
	async getStoreProducts(filters: GetStoreProductsFilters): Promise<StoreProduct[]> {
	...
	async createStoreProduct(storeProductSchema: StoreProductSchemaType): Promise<StoreProduct> {
	...
	async updateStoreProduct({
		storeProductUPC,
		...productSchema
											}: {storeProductUPC: string} & StoreProductSchemaType): Promise<StoreProduct> {
		const response = await fetch(`${API_BASE_URL}store-product/${storeProductUPC}`, {
			method: 'PATCH',
			headers: {
				'Content-Type': 'application/json',
			},
