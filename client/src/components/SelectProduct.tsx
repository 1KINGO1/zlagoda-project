	...
	return (
		<Popover open={open} onOpenChange={setOpen}>
			<PopoverTrigger asChild>
				...
				>
					{value
						? products.find((product) => product.id_product === value)?.product_name
						: "Select product..."}
			...
			<PopoverContent className="p-0">
				<Command>
					<CommandInput
						...
						className="h-9"
						onValueChange={debounce((value: string) => setSearchValue(value), 300)}
					/>
					<CommandList>
						...
						<CommandGroup>
							{products.map((product) => (
								...
								>
									...
									<Check
										className={cn(
											value === product.id_product ? "opacity-100" : "opacity-0"
										)}
									/>
								</CommandItem>
							))}
