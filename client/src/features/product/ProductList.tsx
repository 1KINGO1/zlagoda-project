...
export const ProductList = () => {
	...
	const {data: products, isSuccess: isProductsLoaded} = useProducts({name, category_number, sort});
	...
	return useMemo(() => {
		...
		return products!.map((product) => (
			<TableRow key={product.id_product}>
				...
				<TableCell>{categoriesNameMap.get(product.category_number) ?? "Unknown"}</TableCell>
				<TableCell className="text-right truncate max-w-0">{product.characteristics}</TableCell>
