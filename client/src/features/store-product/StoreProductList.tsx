...
export const StoreProductList = () => {
	const {sortByAmount, sortByName, promotionalProduct} = useStoreProductFilter();
	const {openModal} = useStoreProductModal();
	const {data: storeProducts, isSuccess: isStoreProductsLoaded} = useStoreProducts({
		productInfo: true,
		sortByName,
		sortByAmount,
		promotionalProduct,
	...
	return useMemo(() => {
		...
		return storeProducts!.map((storeProduct) => (
			<TableRow key={storeProduct.upc}>
				...
				<TableCell className="text-center">{storeProduct.selling_price}</TableCell>
				<TableCell className="text-center">{storeProduct.products_number}</TableCell>
				<TableCell className="text-center">{storeProduct.promotional_product ? "TRUE" : "FALSE"}</TableCell>
